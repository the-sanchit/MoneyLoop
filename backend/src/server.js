...
const { readDb, writeDb, getDefaultUserData } = require("./dataStore");
...
app.get("/api/data", authMiddleware, (req, res) => {
  ...
  return res.json(user.data || getDefaultUserData(user.email));
});

app.put("/api/data", authMiddleware, (req, res) => {
  const db = readDb();
  const userIndex = db.users.findIndex((item) => item.id === req.user.userId);
  if (userIndex === -1) {
    return res.status(404).json({ message: "User not found" });
  }
  ...
  db.users[userIndex].data = {
    ...
    settings:
      settings && typeof settings === "object"
        ? {
            username: settings.username || db.users[userIndex].email.split("@")[0] || "Admin",
            currency: settings.currency || "₹",
          }
        : getDefaultUserData(db.users[userIndex].email).settings,
  };
...
app.delete("/api/data", authMiddleware, (req, res) => {
  ...
  db.users[userIndex].data = getDefaultUserData(db.users[userIndex].email);
  writeDb(db);
...
app.post("/api/chat", authMiddleware, async (req, res) => {
  ...
  if (!process.env.GEMINI_API_KEY) {
    ...
    // Expense queries
       ...
       const totalSpent = Array.isArray(data?.expenses) ? data.expenses.reduce((sum, item) => sum + (Number(item.amount) || 0), 0) : 0;
    ...
    // General fallback for all other issues
    else {
